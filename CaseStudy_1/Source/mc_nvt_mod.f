...
c__________________________________________________________________________
     ...
     &        nacc, ncycl, nmoves, imove
      ...
      ! File handle for storing energy values
      INTEGER :: energy_file
      
      ! Open a file to store energy values
      OPEN(unit=energy_file, file='energy_values.txt', status='replace')
...
c        --- ii=2 production
         ...
         attempt = 0
         nacc = 0
c        ---initialize the subroutine that adjusts the maximum displacement
...
c              ---sample averages
               ...
               WRITE(energy_file, '(2F12.5)') en, vir
            END IF
